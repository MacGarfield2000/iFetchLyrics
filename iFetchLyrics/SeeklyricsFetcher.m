...
#import "SeeklyricsFetcher.h"

@implementation SeeklyricsFetcher
...
- (NSString *)_fetchLyricsForArtist:(NSString *)artist album:(NSString *)album title:(NSString *)title {
	...
	title = [title stringByReplacingOccurrencesOfString:@"  " withString:@" "];
	title = [title stringByReplacingOccurrencesOfString:@"  " withString:@" "];
	NSString *urlStr = [[[NSString stringWithFormat:@"http://www.seeklyrics.com/lyrics/%@/%@.html", artist, title] stringByReplacingOccurrencesOfString:@" " withString:@"-"] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
	...
	@try {
		...
		NSString *start = [newline componentsSeparatedByString:@"<div id=\"songlyrics\">"][1];
